...
class Script(UserWindow):
    ...
    def __init__(self):
        ...
        self.user_register.pushButton.clicked.connect(self.user_register_task)  # User register
        self.user_login.pushButton.clicked.connect(self.user_login_task)  # User login

        self.merchant_register.pushButton.clicked.connect(self.merchant_register_task)  # Merchant register
        self.merchant_login.pushButton.clicked.connect(self.merchant_login_task)  # Merchant login

        self.index.tableView.clicked.connect(self.handle_cell_clicked)  # Home page merchant cell clicked event
        self.index.tableView_2.clicked.connect(self.handle_cell_clicked_2)  # Clicking the add to cart button

        self.index.pushButton_3.clicked.connect(self.login_out)  # Log out
        self.index.pushButton.clicked.connect(self.user_shopping_cart_task)  # Clicking the shopping cart button to load data

        self.user_shopping_cart.pushButton_4.clicked.connect(self.index_task)  # User shopping cart back to home page to load data
        self.user_shopping_cart.pushButton_3.clicked.connect(self.login_out)  # Log out
        self.user_shopping_cart.tableView.clicked.connect(self.user_shopping_cart_handle_cell_clicked)  # User shopping cart cell clicked event
        self.user_shopping_cart.pushButton_5.clicked.connect(self.checkout_item_order)  # Checkout

        self.user_update.pushButton.clicked.connect(self.user_update_call)  # Update user information
    ...
    def user_update_call(self): 
        ...
        username, password, email = self.user_update.lineEdit.text(), self.user_update.lineEdit_2.text(), self.user_update.lineEdit_3.text()
        r1 = request('CLIENT_CHECKOUT_ITEM', username)
        r2 = request('SET_CLIENT_PASSWORD', password, password)
        r3 = request('SET_CLIENT_EMAIL', email)
    ...
    def index_task(self):
        """Load data when navigating from the user shopping cart to the home page."""
        self.switch_to_index()
    ...
    def index_merchant_get_data(self):
        ...
        queryset = [[i.tolist()[0], ] for i in request('LIST_MERCHANT')[1].values]
    ...
    def user_shopping_cart_get_data(self):
        ...
        self.user_shopping_cart_model.removeRows(0, self.user_shopping_cart_model.rowCount())
        queryset = [i.tolist() for i in request('CLIENT_GET_ITEMS')[1].values]
        queryset = list(map(lambda i: [i[4], i[0], i[1], i[2], f'{i[3]:.2f}', 'Delete'], queryset))
    ...
    def merchant_register_task(self):
        ...
        response = request('MERCHANT_CREATE', username, description, email, password, yaoqingren, invitation_code)[0]
        self.pop_frame(response)
    ...
    def merchant_index_get_data(self):
        """Retrieve data for the merchant's home page."""
        self.merchant_shouye_model.removeRows(0, self.merchant_shouye_model.rowCount())
        queryset = [i.tolist() for i in request('LIST_PRODUCT', self.merchant_username)[1].values]
        queryset = list(map(lambda i: [i[0], i[1], i[2], i[3], f'{i[4]:.2f}',i[5], 'Replenishment', 'Delete'], queryset))
        ...
        for row_index, row_data in enumerate(queryset):
            for col_index, col_data in enumerate(row_data):
                ...
                if col_index == 6:
                    ...
                    item.setForeground(QColor(Qt.red))
        self.merchant_shouye.label.setText(f'Total profit: {request("MERCHANT_GET_PROFIT")[1]:.2f}')
